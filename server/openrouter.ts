...
export const OPENROUTER_CANDIDATE_MODELS: readonly string[] = [
  "openai/gpt-4o-mini",
...
function compactMessage(message: unknown) {
  return String(message ?? "Unknown failure")
    .replace(/Bearer\s+\S+/gi, "Bearer [redacted]")
    .slice(0, 180);
}
function record(entry: OpenRouterDiagnostic) {
  lifecycle.push(entry);
  if (lifecycle.length > RETENTION)
    lifecycle.splice(0, lifecycle.length - RETENTION);
}
function classify(
  status: number,
  message: string
): OpenRouterDiagnostic["errorType"] {
  if (status === 401 || status === 403) return "auth";
  if (status === 429) return "rate_limit";
  if (status === 400 && /schema|response.?format/i.test(message))
    return "schema";
  return "unknown";
}
...
export function getOpenRouterDiagnostics() {
  return {
    ...
    modelHistory: Object.values(
      lifecycle.reduce<Record<string, OpenRouterDiagnostic[]>>(
        (history, entry) => {
...
export async function invokeOpenRouter(input: {
  ...
  const baseMaxTokens =
    input.maxTokens ?? (input.operation === "generation" ? 1_800 : 1_200);
  record({
    timestamp: started,
    requestId,
    runId,
    ...
    attempt: 1,
    ...(input.operation === "generation" ? { providerSort: "throughput" } : {}),
  });
  const attempts = 2;
  const retryDelayMs = input.retryDelayMs ?? 500;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const compactRetry = input.operation === "generation" && attempt === 2;
    const maxTokens = compactRetry
      ? Math.min(baseMaxTokens, 1_200)
      : baseMaxTokens;
      ...
      ? [
          {
            ...
            content:
              "Retry in concise mode. Preserve all mandatory FRD decisions and identifiers, but use compact tables and short bullet points so the response completes quickly. Do not add a preamble.",
          },
          ...input.messages,
        ]
      : input.messages;
    try {
      const response = await fetch(
        "https://openrouter.ai/api/v1/chat/completions",
        {
          ...
          headers: {
            ...
            "HTTP-Referer": "https://reqtofrd.manus.space",
            "X-Title": "ReqToFRD",
          },
          body: JSON.stringify({
            model,
      ...
      if (!response.ok) {
        ...
        record({
          ...
          errorType: classify(response.status, message),
          message,
          timeoutMs,
          maxTokens,
          attempt,
...
export function markOpenRouterMapped(
  result: {
    requestId: string;
    runId: string;
    model: string;
  ...
  operation: Operation,
  outputMode: OutputMode,
  options: { responseKeys?: string[]; storyCount?: number } = {}
) {
  record({
    timestamp: Date.now(),
