const CONTROL_HEADING = /\b(?:mandatory\s+section\s*1\s*:?\s*)?cover\s*page\b|\bdistribution\s*(?:&|and)\s*sign[\s-]*off(?:\s*table)?\b/i;
const BODY_SECTION_HEADING = /^(?:mandatory\s+section\s*[2-6]\b|revision\s+history\b|executive\s+summary\b|functional\s+requirements?\b|functional\s+requirement\s+item\s+\d+\b|integration\b|out\s+of\s+scope\b|scope\s+boundary\b)/i;
